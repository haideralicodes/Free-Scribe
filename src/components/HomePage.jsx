...
export default function HomePage(props) {
    ...
    async function startRecording(){
        ...
        //create media recorder using the stream
  ...
  return (
    <main className='flex-1 p-4 flex flex-col gap-3 text-center sm:gap-4 md:gap-5
     justify-center pb-20'>
        ...
        <button onClick={recordingStatus === 'recording' ? stopRecording : startRecording} className='specialBtn px-4 py-2 rounded-lg flex items-center text-base justify-between gap-4 mx-auto w-72 max-w-full my-4'>
        ...
        <p className='text-base'>Or <label className='text-blue-400 cursor-pointer hover-text-blue-600 duration-200'>upload 
            <input onChange={(e)=>{
                ...
                setFile(tempFile)
            }} className='hidden' type="file" accept='.mp3, .wave, .png'/></label> mp3 file
        </p>
